...
function goToCities(){
  textArea.value = getStringsByDelimiters("Roteiros para ", "</b>")
}
function goToItinerary(){
  textArea.value = getStringsByDelimiters("#Roteiro A " , "#Roteiro B ")
}
function goToSP(){
  textArea.value = getListOfPlacesByIndex(0,"Paulista","#Roteiro A " , "#Roteiro B ").length
}
function goToLV(){
  textArea.value = getListOfPlacesByIndex(1,"South","#Roteiro A " , "#Roteiro B ").length
}
function goToM(){
  textArea.value = getListOfPlacesByIndex(2,"Vermelha","#Roteiro A " , "#Roteiro B ").length
}
function goToDowntownSP(){
  textArea.value = getListOfPlacesByIndex(0,"Centro","#Roteiro B " , "#Roteiro C ")
}
function goToDowntownLV(){
  textArea.value = getListOfPlacesByIndex(1,"Downtown","#Roteiro B " , "#Roteiro C ") 
}
...
function getStringsByDelimiters(firstDelimiter, lastDelimiter) {
  var splittedHtml = getFormattedHtml().split("<b>->");
  var resultList = [];
  for (i = 0; i < splittedHtml.length; i++) {
    if (splittedHtml[i].includes(firstDelimiter)) {
      var inicialCity =
        splittedHtml[i].search(firstDelimiter) + firstDelimiter.length + 1;
      var lastCity = splittedHtml[i].search(lastDelimiter) - 1;
      resultList.push(splittedHtml[i].substring(inicialCity, lastCity));
    }
  }
  return resultList;
}

function getListOfPlacesByIndex(
  index,
  splitDelimiter,
  firstDelimiter,
  lastDelimiter
) {
  var list = getStringsByDelimiters(firstDelimiter, lastDelimiter);
  var newList = list[index].split(splitDelimiter)[1].split(";");
  var resultList = []
